...
const limbs_index = [[0,1], [1,16], [2,3], [3,4], [4,17], [5,6], [6,7], [7,18], [8,9], [9,10], [10,19], [11,12], [12,13], [13,20]];
const limbs_color = ['purple', 'purple', 'red', 'red', 'red', 'blue', 'blue', 'blue', 'red', 'red', 'red', 'blue', 'blue', 'blue'];


export class LeafletVideoAnnotation extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            annotating : false // Are we currently annotating something?
        };

        // Control disable new btn
        // have annotation disable otherwise enable new btn
        this.hideNewBtn = this.state.annotations.length > 0 ? 'disabled' : '' ;
        ...
        // Basically, it type same with annotation_layers but it will updated by every operation
        // e.g. change visibility annotation_layers will not do anything, so cannot use to draw limbs
        this.limbs_layers = null;
    ...
    componentDidMount() {
        ...
        let image = L.imageOverlay(this.props.imageSrc, bounds).addTo(leafletMap);
        ...
        // Draw limbs layer hust for viualize
        this.polylines_limbs = new L.layerGroup().addTo(leafletMap);
        ...
        for(var i=0; i < this.state.annotations.length; i++){
            this.annotation_layers.push(this.addAnnotation(this.state.annotations[i], i, this.props.categories[0]));
        }

        // do a deep copy
        this.limbs_layers = $.extend(true, [], this.annotation_layers);
    ...
    updateCurrentLeafletMap() {
        ...
        // Add the feature group that will hold the annotations
        // All layers added to this feature group will be editable
        this.annotationFeatures.clearLayers();

        // Initialize the editor
        this.editor = new L.EditToolbar.Edit(leafletMap, {featureGroup : this.annotationFeatures});

        // Draw limbs layer hust for viualize
        this.polylines_limbs.clearLayers();
        ...
        for(var i=0; i < this.props.annotations.length; i++){
            this.annotation_layers.push(this.addAnnotation(this.props.annotations[i], i, this.props.categories[0]));
        }
     ...
     */
    addAnnotation(annotation, annotationIndex, category) {
        ...
        if(annotation.keypoints != 'undefined' && annotation.keypoints != null){
            ...
            let keypoint_names = category['keypoints_name'];
            let keypoint_categorys = category['keypoints_style'];
            // if(category != null){
            ...
            for( var i = 0; i < annotation.keypoints.length / 3; i++){
                ...
                let keypoint_color = keypoint_categorys[i];
                ...
                if (v > 0){
                    ...
                    x = x * imageWidth;
                    y = y * imageHeight;
                    latlng = this.leafletMap.unproject([x,y], 0);
                    ...
                    // Now only allow v=0 and v=2, v=1 occlude disable
                    // if (v == 1){
                    //     marker.setBackground(this.createKeypointStripedBackgroundStyle(keypoint_color));
                    // }

                    // annot_idx is a custom property
                    // it is for moving anf draw stop to re draw the kps
                    // when draw or move, only get one marker data. We need more extra data.
                    layer = L.marker(latlng, {icon : marker});
                    layer.bindTooltip(keypoint_name, {
                        className : '',
                        direction : 'auto',
                        annot_idx : annotationIndex
                    });
                ...
                // all x,y coords of keypoints store in this array for plotting limbs
                // if v=0, still store null in array.
                latlngs.push(latlng);
            }

            // draw limbs
            this.drawLimbs(latlngs);
        }
    ...
    /**
     * Draw night of limbs
     * Now only support one person limbs
     * @param latlngs
     */
    drawLimbs(latlngs) {

        for (var idx = 0; idx < limbs_index.length; idx++) {

            var start_point = limbs_index[idx][0];
            var end_point = limbs_index[idx][1];

            // both start and end point need to be v=2
            // if start and end point either null, the limbs should not exist
            if (latlngs[start_point] && latlngs[end_point]) {

                var line_layer = L.polyline([], {color: limbs_color[idx]});

                line_layer.addLatLng(latlngs[start_point]);
                line_layer.addLatLng(latlngs[end_point]);

                this.polylines_limbs.addLayer(line_layer);
            }
        }
    }
    ...
    _drawStopEvent(e) {
        ...
        if(this.state.annotating && !this._drawSuccessfullyCreated){
            ...
            // For new a instance
            this.polylines_limbs.clearLayers();
            this.drawLimbs(this.extractLatlngsFromLimbsLayer(this.current_annotationIndex));
        }
        else{
    ...
    _drawCreatedEvent(e) {
        ...
        if(this.annotating_keypoint){
            ...
            // console.log(this.annotation_layers);
            // console.log(this.limbs_layers);

            // Save off the layer
            this.annotation_layers[this.current_annotationIndex]['keypoints'][this.current_keypointIndex] = layer;
            this.limbs_layers[this.current_annotationIndex]['keypoints'][this.current_keypointIndex] = layer;
            ...
            let keypoint_color = this.props.categories[0]['keypoints_style'][this.current_keypointIndex];
            if (visibility == 1){
            ...
            let keypoint_name = this.props.categories[0]['keypoints_name'][this.current_keypointIndex];
            layer.bindTooltip(keypoint_name, {
                className : '',
                direction : 'auto',
                annot_idx : this.current_annotationIndex
            });
    ...
    extractLatlngsFromLimbsLayer(annotationIndex) {

        let latlngs = [];
        let annotationIdx = null;

        // temporary only single person, so use 0 index
        // update latlngs from annotation_layers(contain moved x,y info)
        // console.log(this.limbs_layers);
        ...
        if (annotationIndex != null) {
            ...
            annotationIdx = this.current_annotationIndex;
        }


        if (this.limbs_layers[annotationIdx]) {
            for (var kp of this.limbs_layers[annotationIdx].keypoints) {

                // check layer kp obj first. if v=0, obj will return null
                if (kp) {
                    latlngs.push(kp._latlng)
                } else {
                    latlngs.push(null);
                }
            }
    ...
    _layerMoved(e){
        // edit corresponding limbs
        this.polylines_limbs.clearLayers();
        this.drawLimbs(this.extractLatlngsFromLimbsLayer(e.layer.getTooltip().options.annot_idx));
    }
    ...
    handleKeypointVisibilityChange(annotationIndex, keypointIndex, visibility){
        ...
        if (visibility == 0){
            ...
            if (this.state.annotating){
                ...
                if (prev_visibility > 0){
                    ...
                    this.limbs_layers[annotationIndex]['keypoints'][keypointIndex] = null;
            ...
            if (this.state.annotating){
                ...
                if (this.current_annotationIndex == annotationIndex && this.annotating_keypoint && this.current_keypointIndex == keypointIndex){
                    ...
                    if (this._currentDrawer._marker != undefined){
                        ...
                        // let category = this.categoryMap[annotation['category_id']];
                        let keypoint_color = this.props.categories[0]['keypoints_style'][keypointIndex];
                        if (visibility == 1){
                ...
                console.log('Visibility change from 2 -> 0');
                ...
                if(keypoint_layer != null){
                    ...
                    if (prev_visibility == 0){
                        console.log('resume kp');

                        // just render the existing layer
                        this.addLayer(keypoint_layer);
                        this.limbs_layers[annotationIndex]['keypoints'][keypointIndex] = keypoint_layer;
                    }
                    ...
                    console.log('annotate kp');
                    // we need to annotate this keypoint
                    // let annotation = this.state.annotations[annotationIndex];
                    // let category = this.categoryMap[annotation['category_id']];

                    let keypoint_name = this.props.categories[0]['keypoints_name'][keypointIndex];
                    let keypoint_color = this.props.categories[0]['keypoints_style'][keypointIndex];

                    // Set the annotating state
                    this.annotateKeypoint(keypoint_color, keypoint_name, visibility, annotationIndex);
        ...
        // edit corresponding limbs
        this.polylines_limbs.clearLayers();
        this.drawLimbs(this.extractLatlngsFromLimbsLayer(annotationIndex));
    }
    ...
    addNewInstance(category_idx){
        ...
        // Draw kp, skip bbox
        // This is a new instance. Grab the category that was chosen by the user for the new instance.
        let category = this.categoryMap[this.props.categories[category_idx].id];

        // Initialize the keypoints with default values.
        var keypoints = null;
        var keypoint_layers = null;

        if(category['keypoints_name'] && category['keypoints_name'].length > 0){
            keypoints = [];
            keypoint_layers = [];
            for(var j = 0; j < category['keypoints_name'].length; j++){
                var [x, y, v] = [0, 0, 0];
                keypoints.push(x);
                keypoints.push(y);
                keypoints.push(v);
        ...
        var annotation = {
            ...
            'category_id': category.id,
            'keypoints' : keypoints,
            'num_keypoints' : category['keypoints_name'].length
        };
        ...
        this.annotation_layers.push(annotation_layer);
        this.limbs_layers.push($.extend(true, {}, annotation_layer));
    ...
    createNewInstance() {
        ...
        // check it can new a kps or not
        this.hideNewBtn = 'disabled';
    ...
    handleAnnotationDelete(annotation_id){
        ...
        this.limbs_layers[annotation_id] = null;
        ...
        // after delete the kps, clear the limbs and limbs data
        this.polylines_limbs.clearLayers();
        this.hideNewBtn = '';
    }
    ...
    getAnnotations(){
        ...
        for (var i =0; i < annotations.length; i++){
            ...
            if(annotation_layer['keypoints'] != 'undefined' && annotation_layer['keypoints'] != null){
                ...
                // let category = this.categoryMap[annotation['category_id']];
                for(var j=0; j < this.props.categories[0]['keypoints_name'].length; j++){
                    let index = j * 3;
    ...
    handleAnnotateKeypoints(annotationIndex){
        ...
        if(this.props.categories[0]['keypoints_name']){
            ...
            <div>
                <div className="row">
                    <div className="col-md-8 col-xs-12">
                        <div className="row">
                            <div className="col">
                                <div ref={ e => { this.leafletHolderEl = e; }} className='leaflet-image-holder' style={{width: '100%'}}></div>
                            </div>
                        </div>
                        <div className="row">
                            <div className="col">
                    ...
                    <div className="col-md-4 col-xs-12">
                        <div className="d-flex justify-content-between">
                            <div className="p-2">
                                <button type="button" className="btn btn-outline-primary" disabled={this.hideNewBtn} onClick={this.createNewInstance}>New</button>
                            </div>
                            <div className="p-2">
                                <div className="btn-group" role="group">
                                    <button type="button" className="btn btn-outline-secondary" disabled onClick={this.hideAllAnnotations}>Hide All</button>
                                    <button type="button" className="btn btn-outline-secondary" disabled onClick={this.showAllAnnotations}>Show All</button>
                                </div>
                            </div>
                            <div className="p-2">
                                <button id="save" type="button" className="btn btn-outline-success" onClick={this.handleSave}>Save</button>
                            </div>
                        </div>
                        <div className="row">
                            <div className="col">
                ...
                {/* get more height to over the navbar */}
